...
export function extendShader<T extends Instance>(shaderIO: IShaderInitialization<T>, vs?: IShaderExtension, fs?: IShaderExtension) {
  ...
  if (vs) {
    ...
    options[templateVars.extendHeader] = vs.header ? `${vs.header}\n$\{${templateVars.extendHeader}}` : '';
    options[templateVars.extend] = vs.body ? `${vs.body}\n$\{${templateVars.extend}}` : '';
    ...
    extendedVS = shaderTemplate({
      ...
      onToken: (token, replace) => {
        if (token === templateVars.extendHeader || token === templateVars.extend) {
          return replace;
        }
  ...
  if (fs) {
    ...
    options[templateVars.extendHeader] = fs.header ? `${fs.header}\n$\{${templateVars.extendHeader}}` : '';
    options[templateVars.extend] = fs.body ? `${fs.body}\n$\{${templateVars.extend}}` : '';
