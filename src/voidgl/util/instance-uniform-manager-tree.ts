...
import { makeInstanceUniformName } from './make-instance-uniform-name';

export interface IUniformInstanceCluster {
  /** This is the index of the instance as it appears in the buffer */
  instanceIndex: number;
  /** These are the uniforms associated with the cluster */
...
export class InstanceUniformManager<T extends Instance> {
  ...
  private makeNewBuffer() {
    ...
    (newMaterial as any).program = (this.layer.material as any).program;
    ...
    for (let i = 0, end = this.layer.maxInstancesPerBuffer; i < end; ++i) {
      ...
      for (let k = 0, endk = this.uniformBlocksPerInstance; k < endk; ++k) {
        const uniformName = makeInstanceUniformName(uniformIndex);
        uniformIndex++;
        cluster.uniforms.push(newMaterial.uniforms[uniformName]);
      }
    ...
    // Grab the global uniforms from the material and add it to the uniforms materialUniform list
