...
void main() {
  ...
  vec2 s = (anchor + start * min(scaleFactor, maxScale) / scaleFactor);
  vec2 e = (anchor + end * min(scaleFactor, maxScale) / scaleFactor);

  // Get the position of the current vertex
  vec2 currentPosition = interpolation(interpolationTime, s, e, control.xy, control.zw);
  // Calculate the next and previous segment's location on the line
  vec2 prePosition = interpolation(interpolationTime - interpolationIncrement, s, e, control.xy, control.zw);
  vec2 nextPosition = interpolation(interpolationTime + interpolationIncrement, s, e, control.xy, control.zw);
  ...
  float lineThickness = mix(thickness.x , thickness.y, interpolationTime)  / (2.0 * scaleFactor);
