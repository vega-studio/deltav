import { CircleInstance, CircleLayer, DataProvider, extendShaderInitialization, ICircleLayerProps, IShaderInitialization, UniformSize } from '../../../src';

export interface IExtendedCirclesProps extends ICircleLayerProps<CircleInstance> {
  dimming?: number;
}

export class ExtendedCircles extends CircleLayer<CircleInstance, IExtendedCirclesProps> {
  ...
  initShader(): IShaderInitialization<CircleInstance> {
    const shaderIO = extendShaderInitialization(
      super.initShader(),
      {
        uniforms: [
          {
