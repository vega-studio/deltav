...
import { IPipeline, isString, Lookup, Omit, Size, SurfaceErrorType } from "../types";
 ...
 */
function lookupValues<T extends object>(check: Function, lookup: Lookup<T> | T[]): T[] {
  const out: T[] = [];
 ...
 * This gets all of the values of a Lookup
 */
function mapLookupValues<T extends object, U>(
  label: string,
...
export interface IBasicSurfaceOptions<
  ...
  TEvents extends Lookup<EventManager>,
  TResources extends Lookup<BaseResourceOptions>,
...
> {
  ...
  async init() {
    ...
    try {
      ...
      if ((err as any).error === SurfaceErrorType.NO_WEBGL_CONTEXT) {
  ...
  fitContainer(preventRedraw?: boolean) {
    if (this.base) {
      ...
      if (this.context) this.options.container.appendChild(this.context);
  ...
  setContainer(container: HTMLElement) {
    ...
    if (this.context) container.appendChild(this.context);
    this.fitContainer();
  }
