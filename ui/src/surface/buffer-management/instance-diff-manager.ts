...
> {
  ...
  makeProcessor(
    ...
    // If this manager has already figured out which processor to use. Just
    ...
    if (
      ...
      this.processor = new VertexAttributeDiffProcessor(layer, bufferManager);
    } else {
    ...
    // This order matters a LOT as it maps change diff types to the correct
    // processor method. Switching the order of CHANGE, ADD, REMOVE will break
    // all processing.
    this.processing = [
