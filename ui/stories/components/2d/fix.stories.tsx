...
function parseInstructions(d: string[]) {
  return d
    ...
    .map((command) => ({
      ...
      values: command.values.map((v) => parseFloat(v)),
    }));
}
    ...
    async didMount() {
      ...
      const d = paths
        .map((p) => {
          ...
          node.innerHTML = p;
          const childElement = node.childNodes[0] as Element | null;
          return childElement?.getAttribute("d");
        })
        .filter(isDefined);
      ...
      for (let i = 0; i < instructions.length; i++) {
        ...
        switch (instructions[i].command) {
          ...
          case "V":
            // vertical lineto: V y
            start = cursor;
            end = [cursor[0], instructions[i].values[0]];
          ...
          default:
            ...
            console.error("Unsupported d command:", instructions[i]);
            break;
        }
      }
    },
