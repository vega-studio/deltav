...
/**
 * Shows color picking support with one view used for picking and another view
 * for simply rendering the scene. This also shows how some Layers can be
...
export const Color_Picking_Seperate_Views_And_Layers: StoryFn = (() => {
  ...
  useLifecycle({
    async didMount() {
      // Wait for the surface to establish the full pipeline
      const surface = await ready.current.promise;
      if (!circleProviderRed.current || !circleProviderGreen.current) return;
      ...
      for (let i = 0, iMax = 10000; i < iMax; ++i) {
        instances.push(
          circleProviderGreen.current.add(
            new CircleInstance({
              center: [Math.random() * size.width, Math.random() * size.height],
              radius: Math.random() * 5 + 2,
              color: [
  ...
  return (
    ...
    >
      ...
      <SceneJSX name="render">
        ...
        <LayerJSX
          type={CircleLayer}
          providerRef={circleProviderRed}
          config={{}}
        />
        <LayerJSX
          type={CircleLayer}
          providerRef={circleProviderGreen}
          config={{
            picking: PickType.SINGLE,
            animate: {
              radius: AutoEasingMethod.easeOutCubic(200),
