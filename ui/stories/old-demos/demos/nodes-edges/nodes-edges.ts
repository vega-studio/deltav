...
export class NodesEdges extends BaseDemo {
  ...
  center: CircleInstance = new CircleInstance({ center: [0, 0], radius: 10 });
  boundsView: RectangleInstance = new RectangleInstance({});
  ...
  handleCircleClick = (info: IPickInfo<CircleInstance>) => {
    ...
    (this.surface.eventManagers as any).main.centerOn(info.projection.id, [
      focus.center[0],
      focus.center[1],
      0
    ]);
