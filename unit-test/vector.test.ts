...
export function fuzzCompare1(v1: Vec1, v2: Vec1): boolean {
...
export function fuzzCompare4(v1: Vec4, v2: Vec4): boolean {
  return (
    Math.abs(v1[0] - v2[0]) <= 1e-7 &&
    Math.abs(v1[1] - v2[1]) <= 1e-7 &&
    Math.abs(v1[2] - v2[2]) <= 1e-7 &&
...
describe("Vector Library", () => {
  ...
  describe("Direction Vectors", () => {
    ...
    it("Should be +y", () => {
      assert3(up3([0, 0, -1], [0, 1, 0]), [0, 1, 0]);
    });

    it("Should be -y", () => {
      assert3(down3([0, 0, -1], [0, 1, 0]), [0, -1, 0]);
    });
  });
